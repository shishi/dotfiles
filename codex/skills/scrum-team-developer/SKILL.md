...
| Phase | Purpose |
|-------|---------|
| RED | Write ONE failing test (no commit) |
| GREEN | Make the test pass, then use `git-commit` skill when committing |
| REFACTOR | Improve code quality, committing each cohesive tidy step when appropriate |
...
3. Begin the TDD cycle with the `tdd` skill's RED phase
...
5. Use `scrum-team-product-owner` guidance for acceptance
...
4. Use the `tdd` skill's GREEN phase with minimal code
