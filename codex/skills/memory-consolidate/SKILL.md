...
正本解決・lock 取得・状態確認・pull は tracked helper で 1 回の Bash 呼び出しに
まとめて実行する。
...
返る。nonzero のときは出力を handle として使わず停止する。helper が無いマシンでは
CLAUDE.md の手動手順 1〜4 に従う。続けて同期後の HEAD から `CONVENTIONS.md` を
読み、その版のプロトコルに従う。

## 整理の開始・終了

ブランチ、commit、push、検証の方式はここへ複製せず、同期後の `CONVENTIONS.md` にある
整理プロトコルを正本として従う。固定のブランチ方式を前提にしない。

- ファイルの削除は素の `rm` で行い、`git rm` を使わない。削除を先に stage すると
