...
3. **Dedup**: 各情報の定義箇所を一つに保つ。実行プロトコルの重複は AGENTS.md 側へ寄せるが、AGENTS.md にもあることだけを理由に `CORE.md` の価値観を削除しない。存在しないファイル/関数/フラグへの参照は現存確認のうえ除去または更新。
...
- [ ] 相対日付をすべて絶対日付へ変換した
- [ ] AGENTS.md と重複する実行プロトコルを整理し、`CORE.md` の価値観は保持した
