...
- 文書ごとに対象読者を定めてから書き始め、その読者に向けて書く。読者を特定
  できない文書は書かない。対象読者を文書自体に書き込む必要はない
- 個人名は、特に必要がある場合または指示された場合に限り、「誰の名前をどこに
  書くか」をユーザーへ先に宣言してから書く
...
文書種別を決めたら、記述ごとに「これはこの種別に書く内容か」を判定する。
属さない記述は正しい置き場へ移すか削る。追記のときも同じ判定を通す。

- **README**: この repo を使う・運用する読者がその場で必要とする情報だけ
  (目的、正本の所在、手順、動作前提、既知の制限)。将来計画・未実装の構想は
  issue へ。正本ファイルにある値を README へ複製せず、正本への参照にする
