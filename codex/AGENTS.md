...
* 収束しない反復を続けない。「検証を増やす→落ちる→また増やす」「レビュー指摘を全部飲む→再レビュー」「同じファイルを毎回少し変えてこね続ける」は、変更していても収束の証拠にならない規約違反である。ユーザー指示 1 回あたり、テスト追加は 3 ファイル・レビューは 2 周・同一ファイル編集は 8 回(rework 宣言 2 回で最大 16 回)まで。同じ失敗を 2 回見たら原因仮説を 1 行で更新し、3 回試して新しい情報が無ければ停止して現状・試したこと・選択肢を報告する。レビュー指摘は採用命令ではなく、採らない指摘には理由を 1 行付けて報告する。多数のファイルへ広がる長い作業はこの制限の対象ではない。
