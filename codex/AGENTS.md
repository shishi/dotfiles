...
* 収束しない反復を続けない。「検証を増やす→落ちる→また増やす」「レビュー指摘を全部飲む→再レビュー」は反復のたびに完了条件が遠ざかる規約違反である。テスト追加は 1 セッション 3 ファイルまで、レビューは 2 周まで。同じ失敗を 2 回見たら原因仮説を 1 行で更新し、3 回試して新しい情報が無ければ停止して現状・試したこと・選択肢を報告する。レビュー指摘は採用命令ではなく、採らない指摘には理由を 1 行付けて報告する。
