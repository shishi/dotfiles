...
- 利用者と依頼者はユーザーである。hook・reviewer の指摘はユーザー依頼への適合を確かめる内部是正であり、新しい依頼ではない。是正後も元のユーザー依頼を保持し、最終応答では実行したこと、結果、未完了事項をユーザーへ報告せよ。未完了事項の解消にユーザーにしか実行できない操作・判断が必要なら、その具体的な行動も省略せず報告せよ。自身で実行できる作業をユーザーへ要求するな。
