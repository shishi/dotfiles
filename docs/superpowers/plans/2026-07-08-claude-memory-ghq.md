...
old="$HOME/dev/claude-memory"

# 2. ghq root: GHQ_ROOT env → gitconfig ghq.root の先頭値 (ghq の primary root 仕様)
ghq_root="${GHQ_ROOT:-}"
if [ -z "$ghq_root" ]; then
  ghq_root="$(git config --type=path --get-all ghq.root 2>/dev/null | head -n1 || true)"
fi
...
Run: `bash tests/claude-memory-ghq.sh`
Expected: `ok:` × 9(1a/1b/1c/2/3/4/5/6/16)、`PASS=9 FAIL=0`、exit 0
...
Run: `bash tests/claude-memory-ghq.sh`
Expected: `PASS=21 FAIL=0`(Task 1 の 9 + 本タスクの 12: 7/8a/8b×2/8c/9/10/11/12/13/14/15)、exit 0
...
Expected: syntax OK、`tests/claude-memory-ghq.sh` が `PASS=22 FAIL=0`(21 + 順序検証)、Windows テストも全 ok
