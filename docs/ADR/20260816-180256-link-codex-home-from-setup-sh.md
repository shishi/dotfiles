...
Its remedy names a few runtime entries as landmarks, not as a list to work through. What has to move is everything the ignore rule denies under the linked path, credentials first; the re-included tracked files are authoritative in the repository. `claude/memory` is excluded because the same run recreates it. The remedy also opens by telling an operator running from a temporary worktree to do nothing at all: the destination it would otherwise name is the running checkout, and state moved into a worktree goes when the worktree does.

Only the bind-mount case stays outside it. A `~/.claude` provided by a devcontainer bind mount is a real path, and the real-path remedy — move it aside and re-run — is wrong there: the mount already exposes the same contents, and moving it aside acts on the mount source. That check runs first and reports on its own; the shared function never sees the path. Detection can still miss one, so the real-path remedy for `~/.claude` asks for that to be ruled out before anything is moved. `~/.codex` needs no equivalent because nothing bind-mounts it.
