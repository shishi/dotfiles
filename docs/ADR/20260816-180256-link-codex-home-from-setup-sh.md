...
The configuration classification fixtures `config_classification.json` and `live_config_sanitized.toml` move from `codex-tools/tests/fixtures/` to `tests/fixtures/`. Six tests in `tests/codex_managed_state_test.py` read them. One has force over live content: it requires every key in both the tracked `config.toml` and the sanitized snapshot to match a classification pattern, so a key the app newly writes fails the test until it is classified by hand. Four keep that policy precise enough to have teeth, because an over-broad pattern would silently stop catching new keys. The last one only asserts that the two fixtures exist. `expected_managed_config.toml` does not move: it described the output of the deleted classifier, nothing compared a run against it, and the only test reading it asserted properties of the fixture itself.
...
* Nothing consumes the classification decisions at runtime, so the fixture guard only forces a human to classify a newly written key; it cannot act on the classification.
