...
mkdir "$WORK/run.claim" 2>/dev/null || exit 96          # 使用済み = 既に委譲を起動している
...
| 96 | 作業ディレクトリの取り違え(`prompt.md` が無い、または既に委譲を起動済み) | 1 回目が返したパスを貼り直す。起動済みなら 1 からやり直す |
...
受け取る量の上限は schema の `maxLength` と `maxItems` が決める。**文字数では合計 22,600 文字**
(`summary` 600 + `changed_files` 50×200 + `findings` 20×300 + `next_steps` 10×300 +
`blockers` 10×300)。これは実際に効く上限で、これを超える出力を求めても各項目がこの値で切られる。

**バイト数はこれより大きい。** `maxLength` が数えるのは文字であってバイトではないため、日本語の
本文なら 1 文字 3 バイトで最大およそ 68,000 バイトになる。JSON のキー・引用符・区切りが足すのは
数百バイトで、効くのはこちらではない。委譲を 10 本流しても context 増分がこの 10 倍を超えないことは
言えるが、その 10 倍を「23 KB の 10 倍」と見積もると外す。
