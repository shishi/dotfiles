...
受け取る量の上限は schema の `maxLength` と `maxItems` が決める。**上限が掛かるのは自由記述の
5 項目の文字数で、その合計は 22,600 文字**(`summary` 600 + `changed_files` 50×200 +
`findings` 20×300 + `next_steps` 10×300 + `blockers` 10×300)。この上限は実際に効き、これを超える
出力を求めても各項目がこの値で切られる。`status` は enum なので上限の対象外で、最長の
`blocked` でも 7 文字である。
