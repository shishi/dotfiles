...
case "$WORK" in (*/codex-delegate-*) ;; (*) exit 96 ;; esac  # 1 回目が作った形でないパスは触らない
...
- **パスの形を照合してから `rm -rf` の射程に入れる。** `trap` が消すのは変数の指す先なので、
  ...
  いる以上、これは到達する経路である。1 回目が作る名前(`codex-delegate-` を含む)に合わない
  パスでは、`prompt.md` の有無を見る前に止まる。`prompt.md` という名前のファイルは委譲の作業
  ディレクトリに限らず存在しうるため、その存在だけでは射程を絞れない
...
受け取る量の上限は schema の `maxLength` と `maxItems` が決める。**上限が掛かるのは文字列の値の
文字数で、合計 22,600 文字**(`summary` 600 + `changed_files` 50×200 + `findings` 20×300 +
`next_steps` 10×300 + `blockers` 10×300)。この上限は実際に効き、これを超える出力を求めても各項目が
この値で切られる。

**22,600 は受け取るデータ量の上限ではなく、見積もりに使う数である。** `maxLength` が数えるのは
値をデコードしたあとの文字数で、実際に届くのは JSON にシリアライズした形だから、次の 3 つがこれに
上乗せされる。

- **文字のバイト数。** 日本語の本文なら 1 文字 3 バイトで、同じ 22,600 文字がおよそ 68,000 バイトになる
- **エスケープ。** 本文の `"` と `\` は 2 文字に、改行と制御文字は `\n` や ` ` の形に置き換わる。
  上限は置換前の文字数に掛かるので、置換で増えた分は上限の外側にある
- **JSON の構造。** キー・引用符・区切りで数百バイト。3 つのうちこれだけが小さい

厳密な受信量を見たいときはシリアライズ後のバイト数(`wc -c` を通した `out.json` の大きさ)で測る。
委譲を 10 本流しても context 増分が 1 本の 10 倍を超えないことは言えるが、その 1 本を 23 KB と
見積もると外す。

これは context 増分の上限であって、委譲が消費するトークン全体の上限ではない。
